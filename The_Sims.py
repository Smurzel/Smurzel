...
class Human:
    ...
    def feed_pet(self, Pet):
        ...
        if Pet.energy > 100:
            Pet.energy = 100
        print(f"{self.name} fed {Pet.name}.")
    def play_with_pet(self, Pet):
        Pet.energy -= 25
        Pet.satiety -= 10
        if Pet.energy == 0:
            print(f"{Pet.name} too tired to play with {self.name}.")
        print(f"{Pet.name} played with {self.name} and got tired.")
